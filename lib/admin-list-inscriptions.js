...
//
// Columnas necesarias para el panel. stripe_session_id se incluye únicamente
// para mostrar "Eliminar prueba" cuando AMBOS IDs son cs_test_;
// no se exponen payment_intent_id / stripe_event_id.
...
const ADMIN_COLUMNS =
  'id, full_name, email, buyer_email, order_session_id, stripe_session_id, ticket_index, ticket_count, event_slug, distance, amount_paid, payment_status, registration_status, cancellation_type, bib_number, cancelled_bib_number, cancelled_at, cancelled_by, cancellation_reason, archived_at, archived_by, archive_reason, shirt_size, birth_date, whatsapp, state, borough, email_sent, created_at';
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (status === 'cancelled') {
      ...
      query = query.eq('registration_status', 'archived');
    } else if (status === 'all') {
      // "Todos" conserva la operación diaria limpia; archivados tienen su filtro propio.
      query = query.neq('registration_status', 'archived');
    } else {
      query = query.eq('payment_status', status).eq('registration_status', 'active');
    }
    if (event !== 'all') {
